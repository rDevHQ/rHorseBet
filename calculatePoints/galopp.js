...
export function calculatePointsForGalopp(race, gameType) {
    ...
    const horses = validHorses.map(h => {
        ...
        // console.log(`🐎 ${h.horse?.name} H2H points:`, headToHeadPoints, 'Meetings:', meetings?.length);
        if (!Number.isFinite(headToHeadPoints)) {
            console.warn(`⚠️ Ogiltig H2H-poäng för ${h.horse?.name}:`, headToHeadPoints);
        }

        const totalPoints =
            bettingPoints + driverPoints + trainerPoints + formPoints + weightAdjustedRatingPoints + equipmentPoints.points + headToHeadPoints + earningsPerStartCurrentYearPoints + earningsPerStartLastTwoYearsPoints;
    ...
    horses.forEach(h => {
        h.mlPoints =
            ...
            ML_CATEGORY_WEIGHTS.Plustal * h.weightAdjustedRatingPoints +
